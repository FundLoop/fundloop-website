...
import { getZkasDatasetDetail } from "@/app/actions/zkas-actions"
import { DatasetStatusBadge } from "@/components/zkas/dataset-status-badge"
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table"

export default async function ProjectZkasDatasetDetailPage({
  params,
}: {
    ...
    <div className="container mx-auto space-y-8 px-4 py-12">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600 dark:text-slate-300">Validation history for dataset #{data.detail.dataset.id}</p>
        </div>
        <Button asChild variant="outline">
          <Link href={`/projects/${slug}/zkas`}>Back to Project zkAS</Link>
        </Button>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Errors block operator approval; warnings are advisory.</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
