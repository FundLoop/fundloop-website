...
export async function observeBaseIntakeV2Receipt(input) {
  ...
  for (const log of receipt.logs) {
    if (log.address.toLowerCase() === snapshot.contractAddress.toLowerCase()) {
      try {
        const decoded = decodeEventLog({ abi: [receiptEvent], data: log.data, topics: log.topics })
        const a = decoded.args
        eventMatched ||= decoded.eventName === "BaseReceipt" && a.projectId === snapshot.projectId &&
          ...
          a.projectFeeBps === snapshot.feeBps && a.feeAmount === snapshot.platformFeeAmount && a.netEpochAmount === snapshot.netEpochAmount &&
  ...
  const evidence = `${receipt.transactionHash}:${receipt.blockHash}:${currentBlockNumber}:${platformObserved}:${epochObserved}`
  ...
  return { receiptId: snapshot.id, currentBlockNumber: Number(currentBlockNumber), observedBlockHash: receipt.blockHash,
    ...
    receiptEventMatched: eventMatched }
}
