...
BEGIN
 ...
 PERFORM public.record_custody_reconciliation(custody,asset,1000,998,2,now(),repeat('e',64),'local');
 IF(SELECT variance_classification FROM public.custody_reconciliation_snapshots ORDER BY id DESC LIMIT 1)<>'within_tolerance'THEN RAISE EXCEPTION 'variance not derived';END IF;
