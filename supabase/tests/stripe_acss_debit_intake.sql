...
DECLARE v_actor uuid:='290eb647-f25f-43f3-bf6b-1e2b2cf25e69';v_payment bigint;v_command uuid;v_command_replay uuid;v_evidence bigint;
  ...
  v_ledger bigint;v_reversal bigint;v_package bigint;v_cycle bigint;v_dataset bigint;v_source_row bigint;v_fx_observation bigint;v_prep_count integer;v_denied boolean:=false;v_direct_denied boolean:=false;
  v_base jsonb;
BEGIN
  ...
  VALUES(v_dataset,3,v_cycle,1,'pad-prep-fixture',v_actor,1) RETURNING id INTO v_source_row;
  ...
  v_fx_observation:=public.record_epoch_fx_observation(jsonb_build_object('contractVersion','epoch_fx_observation.v1','deploymentEnvironment','local',
    'cycleKey','2026-08','assetKey','stripe_acss_cad','sourceKey','pad_primary_fixture','sourceRank',1,'rateUsdPerUnit','0.75',
  ...
  PERFORM public.post_epoch_fx_snapshot(jsonb_build_object('contractVersion','epoch_fx_snapshot.v1','deploymentEnvironment','local',
    'cycleKey','2026-08','assetKey','stripe_acss_cad','method','primary','observationId',v_fx_observation,'preanalysis',jsonb_build_object('fresh',true),
  ...
  IF v_prep_count<>1
    OR NOT EXISTS(SELECT 1 FROM public.epoch_valuation_source_lots lot JOIN public.financial_assets asset ON asset.id=lot.financial_asset_id
      ...
      WHERE lot.package_id=v_package AND lot.source_kind='stripe_acss_debit' AND asset.asset_key='stripe_acss_cad'
        AND custody.custody_key='stripe_acss_cad_clearing' AND lot.native_atomic_amount=10000 AND lot.gross_exact_usd=75)
    OR NOT EXISTS(SELECT 1 FROM public.epoch_funded_allocation_lock_candidates WHERE package_id=v_package AND source_kind='stripe_acss_debit') THEN
    RAISE EXCEPTION 'settled_pad_did_not_reach_financial_prep count %, lot %',v_prep_count,
