import { validateProjectInvitationListInput } from "../../../lib/edge-functions/project-invitation-contract.ts"
import { edgeCommandFailure, edgeCommandSuccess } from "../../../lib/edge-functions/result.ts"
import { executeProjectInvitationList } from "../../../lib/invitations/project-invitation-command.ts"
...
async function handleRequest(request: Request) {
  ...
  const validation = validateProjectInvitationListInput(body.ok ? body.body : undefined)
  ...
  const result = await executeProjectInvitationList(auth.adminClient, { ...validation.data, actorUserId: auth.user.id })
