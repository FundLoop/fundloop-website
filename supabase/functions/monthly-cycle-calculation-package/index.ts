import { validateMonthlyCycleCalculationPackageInput } from "../../../lib/edge-functions/monthly-cycle-calculation-package-contract.ts"
import { edgeCommandFailure, edgeCommandSuccess } from "../../../lib/edge-functions/result.ts"
import { isInternalAdminEmail } from "../../../lib/internal-admin-emails.ts"
import { executeMonthlyCycleCalculationPackageCommand } from "../../../lib/monthly-cycles/monthly-cycle-calculation-package-command.ts"
...
async function handleRequest(request) {
  ...
  const validation = validateMonthlyCycleCalculationPackageInput(bodyResult.body)
  if (!validation.ok) {
    return json(validation)
  }

  const result = await executeMonthlyCycleCalculationPackageCommand(auth.adminClient, {
