import { validateProjectInvitationRevokeInput } from "../../../lib/edge-functions/project-invitation-contract.ts"
import { edgeCommandFailure, edgeCommandSuccess } from "../../../lib/edge-functions/result.ts"
import { executeProjectInvitationRevoke } from "../../../lib/invitations/project-invitation-command.ts"
...
async function handleRequest(request: Request) {
  ...
  const validation = validateProjectInvitationRevokeInput(body.ok ? body.body : undefined)
  ...
  const result = await executeProjectInvitationRevoke(auth.adminClient, {
    invitationId: validation.data.invitationId, actorUserId: auth.user.id, reviewRuntimeEnabled: reviewRuntimeEnabled(),
  })
