import { validateMonthlyCyclePayoutIntentsCreateInput } from "../../../lib/edge-functions/monthly-cycle-payout-intents-create-contract.ts"
import { edgeCommandFailure, edgeCommandSuccess } from "../../../lib/edge-functions/result.ts"
import { isInternalAdminEmail } from "../../../lib/internal-admin-emails.ts"
import { executeMonthlyCyclePayoutIntentsCreateCommand } from "../../../lib/monthly-cycles/monthly-cycle-payout-intents-command.ts"
...
async function handleRequest(request) {
  ...
  const validation = validateMonthlyCyclePayoutIntentsCreateInput(body.body)
  if (!validation.ok) return json(validation)

  const result = await executeMonthlyCyclePayoutIntentsCreateCommand(auth.adminClient, {
    ...validation.data,
