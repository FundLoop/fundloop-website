import { validateMonthlyCycleApprovalInput } from "../../../lib/edge-functions/monthly-cycle-approval-contract.ts"
import { edgeCommandFailure, edgeCommandSuccess } from "../../../lib/edge-functions/result.ts"
import { isInternalAdminEmail } from "../../../lib/internal-admin-emails.ts"
import { executeMonthlyCycleApprovalCommand } from "../../../lib/monthly-cycles/monthly-cycle-verification-command.ts"
...
async function handleRequest(request) {
  ...
  const validation = validateMonthlyCycleApprovalInput(body.body)
  if (!validation.ok) return json(validation)
  const result = await executeMonthlyCycleApprovalCommand(auth.adminClient, {
    ...validation.data,
