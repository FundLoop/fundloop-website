import { validateMonthlyCycleVerificationReviewInput } from "../../../lib/edge-functions/monthly-cycle-verification-review-contract.ts"
import { edgeCommandFailure, edgeCommandSuccess } from "../../../lib/edge-functions/result.ts"
import { isInternalAdminEmail } from "../../../lib/internal-admin-emails.ts"
import { executeMonthlyCycleVerificationReviewCommand } from "../../../lib/monthly-cycles/monthly-cycle-verification-command.ts"
...
async function handleRequest(request) {
  ...
  const validation = validateMonthlyCycleVerificationReviewInput(body.body)
  if (!validation.ok) return json(validation)
  const result = await executeMonthlyCycleVerificationReviewCommand(auth.adminClient, {
    ...validation.data,
