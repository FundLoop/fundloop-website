...
async function handleRequest(request: Request) {
  ...
  if (!["local", "development", "dev", "preview", "test"].includes((getEnv("FUNDLOOP_DEPLOYMENT_ENV") ?? "production").trim().toLowerCase())) {
    return json(edgeCommandFailure("production_disabled", "Canadian PAD status is unavailable in this environment."))
  }
  ...
  const {data, error} = await auth.adminClient.rpc("list_project_stripe_acss_debit_status", {p_actor_user_id: auth.user.id, p_project_slug: projectSlug})
  if (error) return json(edgeCommandFailure("stripe_acss_status_failed", error.message))
