...
describe("four epoch allocation lifecycle executable contract", () => {
  it("models E minus three through E with partial claim protection and released re-eligibility", () => {
    ...
    expect(setup).toContain("cs_test_fourepochcurrent")
    expect(setup).not.toMatch(/(?:cs_test|evt|pi|ch|txn)_four_epoch_current/)
    expect(setup).toMatch(/v_eur_source_lot[\s\S]*v_current_source_lot=v_eur_source_lot/)
    expect(setup).toMatch(/pool\.origin_disposition_id=disposition\.id[\s\S]*source\.source_lot_id=v_eur_source_lot/)
    expect(setup).toMatch(/accounting_period_id=v_target_period[\s\S]*monthly_cycle_id=v_target[\s\S]*current_stage='reviewing'/)
    expect(setup).toMatch(/CASE lot\.rail_key[\s\S]*END\)::public\.payout_rail/)
  })
  ...
  it("runs a genuine two-session claim/harvest race and rejects the stale selection", () => {
    ...
    expect(wrapper).toContain("-v preview_hash=\"$stale_preview_hash\"")
    expect(wrapper).not.toContain("-v preview_hash=\"$stale_preview_hash\" -c")
    ...
    expect(wrapper).toContain("node --no-warnings --experimental-strip-types")
    expect(wrapper).toContain("calculate-four-epoch-allocation.mjs")
    expect(wrapper).not.toContain("pnpm exec tsx")
  ...
  it("asserts cap, source, predecessor, close, report, and root conservation", () => {
    ...
    expect(assertion).toContain("command.provider_checkout_session_id='cs_test_fourepochcurrent'")
