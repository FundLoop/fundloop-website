...
describe("base safe payout operator contract",()=>{
  it("accepts only exact authorize, Safe proof, execute, and observe inputs",()=>{
    ...
    expect(validateBaseSafePayoutOperatorInput({action:"confirm_authorization",commandId:3}).ok).toBe(true)
    ...
    expect(validateBaseSafePayoutOperatorInput({action:"confirm_authorization",commandId:3,authorized:true}).ok).toBe(false)
