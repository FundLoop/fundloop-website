...
describe("Stripe Connect payout migration", () => {
  ...
  it("verifies signed test-mode webhooks and never accepts raw bank input", () => {
    ...
    expect(account).toContain("stripe.accounts.retrieveCurrent()")
    expect(payoutOperator).toContain("stripe.accounts.retrieveCurrent()")
    expect(webhook).toContain("stripe.accounts.retrieveCurrent()")
    expect(`${account}\n${payoutOperator}\n${webhook}`).not.toContain("stripe.accounts.retrieve(null)")
  })
})
