...
test("operator approves the exact result and role surfaces preserve privacy", async ({page,context,browser,baseURL}) => {
  ...
  await page.getByRole("button",{name:"Approve exact result and publish close package"}).click()
  ...
  await expect(publicPage.getByText("Privacy threshold not met")).toBeVisible()
