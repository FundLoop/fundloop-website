...
| `/workspace/earnings` | Signed-in users | Real user earnings and payout workspace. | Issue #83 makes bookkeeping credits the primary credited-but-not-paid earnings source, while published results, payout intents, payout routes, batch status, and reconciliation cues remain future-settlement context. | finish | User earnings and payout workspace | 35, 36, #83 |
| `/workspace/reporting` | Signed-in users | Real user reporting workspace. | Session 36 reads user report metadata and monthly result placeholders so stored report artifacts can explain user outcomes. | finish | User reporting workspace | 36 |
...
| `/founder/projects/[slug]/contributions` | Founders, project members | Founder-facing monthly contribution workflow. | Session 33 groups existing payment obligations by economic month, shows setup/payment-route/attribution readiness, and links into the current payment and zkAS operation routes. | finish | Founder monthly contribution workflow | 33 |
| `/founder/projects/[slug]/attribution` | Founders, project members | Founder-facing contribution-data workflow. | Session 34 exposed the structured zkAS dataset contract and legacy upload handoff. Issue #68 added the MVP manual attribution row form for scoped CUBID identities and typed Edge Function submission while keeping operator approval separate. | finish | Founder project attribution workflow | 34, MVP #68 |
| `/founder/projects/[slug]/reporting` | Founders, project members | Founder-facing reporting workspace. | Session 36 reads founder report metadata and zkAS project summaries for monthly project reporting. | finish | Founder project reporting workflow | 36 |
...
| `/projects/[slug]/payments` | Project admins | Real founder operations surface. | Session 19 moved founder payment-route writes and onchain receipt recording behind typed Edge Function commands while keeping the deep operation URL in place. Session 30 added Solana inbound adapter support behind the shared execution interface; Session 33 now links it from the monthly contribution workflow rather than treating it as the mental model. | finish | Founder workspace project contributions | 18, 19, 30, 33 |
| `/projects/[slug]/zkas` | Project admins, operators | Transitional project-side zkAS page and current dataset upload action surface. | Session 34 wraps it with the founder attribution workflow but leaves dataset upload writes here until a later Edge Function migration. | merge | Founder workspace cycle and reporting area | 33, 34, 36 |
...
| `/admin/cycles/[cycleKey]/prep` | Internal operators | Prep and exception review workspace for locked cycle manifests and MVP attribution review. | Session 23 added blocker/warning review on top of the immutable lock manifest before calculation packaging. MVP issue #69 added internal-operator approval/rejection actions for submitted scoped-CUBID attribution datasets. | finish | Operator monthly cycle prep workspace | 23, 24, 25, MVP #69 |
| `/admin/cycles/[cycleKey]/zkas` | Internal operators | Cycle-anchored zkAS stage view and calculation-package entry. | Session 24 added a monthly-cycle view over linked datasets, identity artifacts, runs, and published outputs; Session 25 added the deterministic package action that creates locked run artifacts. | finish | Operator monthly cycle zkAS stage | 24, 25, 26 |
| `/admin/cycles/[cycleKey]/verification` | Internal operators | Cycle-level cleanup, calculated-result inspection, verification, and approval workspace. | Session 26 added result-total checks, cleanup-needed decisions, verified-cycle review, and approval for distribution. MVP issue #78 added calculated allocator output review for user results, raw project entitlements, asset fills, returned pools, source breakdowns, and result artifacts before approval. | finish | Operator monthly cycle verification workspace | 26, 27, MVP #78 |
...
| `/admin/payments` | Internal operators | Real payments operations hub. | Session 50 moved payment rows, latest onchain submission state, and recent failure summaries behind the shared operator payment workspace read model. | finish | Operator payments workspace | 20, 37, 50 |
...
| `/admin/payments/reconciliation` | Internal operators | Real onchain reconciliation page. | Session 50 moved queue enrichment and partial-read warnings behind the shared operator payment workspace read model. | finish | Operator payments reconciliation | 20, 37, 50 |
...
| `/admin/zkas` | Internal operators | Real zkAS workspace entry. | Session 24 keeps it as the action console but points operators toward monthly cycles for end-to-end readiness. | finish | Operator zkAS workspace, subordinate to monthly cycles | 24 |
| `/admin/zkas/runs` | Internal operators | Real zkAS runs list. | Still owns legacy/manual run creation and execution actions; cycle calculation packaging now starts from `/admin/cycles/[cycleKey]/zkas`. | finish | Operator zkAS runs | 24, 26 |
| `/admin/zkas/runs/[id]` | Internal operators | Real zkAS run detail. | Supports run inspection and status handling; cycle linkage is the canonical grouping. | finish | Operator zkAS run detail | 24, 26 |
| `/admin/zkas/uploads` | Internal operators | Real upload queue/list. | Supports operator prep and triage; cycle stage pages now expose linked readiness per month. | finish | Operator zkAS uploads | 23, 24 |
| `/admin/zkas/uploads/[id]` | Internal operators | Real upload detail. | Supports operator review of submitted artifacts before they enter cycle-linked calculation. | finish | Operator zkAS upload detail | 23, 24 |
...
| `/reports` | Public visitors, transparency readers | Canonical public reporting hub. | Session 36 now lists published public monthly report rows when artifacts exist, while retaining the truthful transparency explanation from Session 11. | finish | Public reporting and transparency hub | 11, 36 |
