...
| `/admin/cycles/[cycleKey]/zkas` | Internal operators | Cycle-anchored zkAS stage view. | Session 24 added a monthly-cycle view over linked datasets, identity artifacts, runs, and published outputs so zkAS is part of the cadence rather than a parallel console. | finish | Operator monthly cycle zkAS stage | 24, 25, 26 |
...
| `/admin/zkas/runs` | Internal operators | Real zkAS runs list. | Still owns run creation/execution actions; future calculation packaging should start from cycle-linked readiness. | finish | Operator zkAS runs | 24, 26 |
