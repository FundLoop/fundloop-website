...
The persona harness is deliberately local-only. The caller starts and resets Supabase/Mailpit; the runner verifies the exact `55321`/`55324` endpoints, owns only a Next process on `127.0.0.1:3002`, and refuses occupied, hosted, or production endpoints before fixture access. Readiness is bounded and classified: Auth, Storage, Mailpit, the service-role-only `persona_goal2_schema_readiness()` candidate identity, every Edge Function required by the selected persona set, an exact per-run Edge commit/nonce/source-contract identity, and an exact per-run Next commit/nonce identity must all answer before fixtures begin. The schema identity is introduced by the final candidate migration and checks the relations, functions, and columns needed by the selected persona and Goal 2 flows; stale contract/migration versions and missing features have separate failure classes. It never relies on an unvalidated HTTP response header.

Selected function inventory is derived from the reviewable checkpoint-to-command registry in `tests/e2e/personas/readiness-boundaries.json`. The registry is rejected if a persona or journey checkpoint is stale, an entrypoint is absent, or a no-function persona lacks explicit read-only route coverage. It includes the full personal and project draft upsert/clear/publish surfaces even when a fixture enters a later review step. The source-contract digest is derived with the same transitive source-closure implementation used by the deployment verifier, so readiness does not duplicate hosted parity rules. `OPTIONS` proves routing only; the separately secret-bound, nonmutating `persona-readiness-identity` command proves that the local Edge process was restarted with the current commit, run nonce, and exact selected-function source digest. Hosted source identity remains the responsibility of the deployment manifest/source-readback verifier after an authorized CI deployment and is not inferred from this local probe.

A delayed healthy dependency is retried. If the local CLI reset leaves Auth and Storage containers healthy but Kong routing stale, one recovery restarts only the validated `supabase_kong_<project_id>` local container, then grants a bounded 90-second recovery window; fixtures never start during that interval. Stale runtime identity, partial schema/function inventory, and dead services fail with distinct reason codes. The readiness evidence is retained in the sanitized run summary. It writes its private recovery ledger and sanitized summaries beneath ignored `output/persona-harness/`.
