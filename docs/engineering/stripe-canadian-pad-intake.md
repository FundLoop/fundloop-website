...
Each project payment may settle through exactly one funding command across Customer Balance, CAD PAD, and Pay by Bank. The first ledger-backed available event claims the payment under a shared advisory lock; a later settlement from another rail rolls back atomically. Package sources must match that immutable claim, the command, its reviewed quote, and the exact native amount.
