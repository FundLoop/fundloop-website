...
Returning from Checkout is never settlement. A verified Stripe signature triggers authoritative SDK reads of the Checkout Session, PaymentIntent, Charge, PaymentMethod, mandate identifier, and BalanceTransaction. Only an ACSS debit with an exact command/session/account/currency/amount/mandate match, a conserved gross = fee + net balance transaction, and `balance_transaction.status=available` creates the provisional custody reference and balanced neutral-ledger receipt. Processing, failure, cancellation, refund, and dispute evidence stays append-only; a full refund or lost dispute reverses the receipt once.
