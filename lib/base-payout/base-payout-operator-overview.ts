...
export async function loadBasePayoutOperatorOverview(monthlyCycleId:number){
  ...
  const [deployments,commands]=await Promise.all([
    ...
    supabase.from("base_payout_execution_commands").select("id,payout_intent_id,status,request_hash,native_atomic_amount,user_fee_native_amount,token_address,recipient_address,fee_recipient_address,authorized_at")
