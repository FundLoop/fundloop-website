...
export async function probeSupabaseFoundation(env, fetcher = fetch) {
  ...
  if (schemaIdentity?.contractVersion !== "fundloop.persona-goal2-schema-readiness.v3" || schemaIdentity?.migrationVersion !== "20260813152000") {
    throw new Error("schema-identity-stale-version")
  }
  if (schemaIdentity?.ready !== true || schemaIdentity?.featureCount !== 36 || !Array.isArray(schemaIdentity?.missingFeatures) || schemaIdentity.missingFeatures.length > 0) {
