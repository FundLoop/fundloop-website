...
export async function probeSupabaseFoundation(env, fetcher = fetch) {
  ...
  if (schemaIdentity?.contractVersion !== "fundloop.persona-goal2-schema-readiness.v2" || schemaIdentity?.migrationVersion !== "20260813150000") {
    throw new Error("schema-identity-stale-version")
  }
  if (schemaIdentity?.ready !== true || schemaIdentity?.featureCount !== 35 || !Array.isArray(schemaIdentity?.missingFeatures) || schemaIdentity.missingFeatures.length > 0) {
